...
def get_symbol(line):
    ...
    elif l > 8 and line[0:7] == "#ifndef":
        return (SYMBOL_IFNDEF, line[8:].strip("\n"))
...
def process(path):
    ...
    for i in data:
        ...
        elif s == SYMBOL_IFNDEF:
            ifndef_current = True
            ifndef_ok = d not in defines
        elif s == SYMBOL_ENDIF:
            ifdef_current = False
            ifndef_current = False
            
        elif (not ifdef_current and not ifndef_current) or (ifdef_current and ifdef_ok) or (ifndef_current and ifndef_ok):
